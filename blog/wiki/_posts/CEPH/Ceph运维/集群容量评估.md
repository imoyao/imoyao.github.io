...
| -rw=randwrite | 表示测试时的读写策略为随机写（random writes）。作其它测试时可以设置为：<br/>1. randread（随机读random reads）<br/> 2. read（顺序读 sequential reads）<br/> 3. write（顺序写 sequential writes <br/> 4. randrw（混合随机读写mixed random reads and writes）|

...
如果所有可能的位置都查询过了但仍有对象丢失，那就得放弃丢失的对象了。这仍可能是罕见的失败组合导致的，集群在写操作恢复后，未能得知写入是否已执行。以下命令把未找到的（ `unfound` ）对象标记为丢失（ `lost` ）。
	...
	ceph pg 2.5 mark_unfound_lost revert|deleteplain
...
osd.1 的日志里也提示修复成功：
	...
	2016-11-10 14:04:31.732640 7f628c5e6700  0 log_channel(cluster) log [INF] : 9.14 repair startsplain
...
2、刷新该 osd 的日志。
	...
	ceph-osd -i xx --flush-journalplainplain

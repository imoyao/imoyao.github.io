...
当 DRBD 因为网络链路异常导致数据复制出错时，首先`drbdadm pause-sync`暂停同步，然后根据资源配置中的超时配置尝试重新建立连接。与此同时，双控内部会去检查心跳口连接，如果心跳口通信正常，则开始内部接管流程：AN 端接管 AO 端的业务，即原来控一上的资源被控二接管。在一定的超时时间后，如果网络没有恢复连接，则开始进行仲裁抢占。

...
----------------------------------------------------------------------------
-c comment 给新用户添加备注
-d home_dir 为用户指定home目录（如果不想用登录名作为主目录名的话）
...
-m 创建用户的HOME目录
-M 不创建用户的HOME目录（当默认设置里指定创建时，才用到）注意：不创建是指不创建真实目录，但是/etc/passwd下面查看的时候，home目录还是存在的。
-N 不要创建一个同用户登录名同名的新组
-r 创建系统账户
...
### `-b`和`-d`的区别
-b指定用户主目录的位置。在普通Linux机器上，这将是`/home`;可以通过编辑`/ etc/default/useradd`更改默认值。 useradd将新用户名添加到此路径以获取主目录。 这意味着如果我们执行：
```bash
...
[Linux命令之useradd - 苦逼运维 - 博客园](https://www.cnblogs.com/diantong/p/9430258.html)